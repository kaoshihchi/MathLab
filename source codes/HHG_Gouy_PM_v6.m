...
ParameterFilename = 'Para_Ne_205th_251009.txt'; 
FigureSwitch = 1; 
...
E_LH_l = zeros(1, N_z);
E_LH_s = zeros(1, N_z);
E_LH_l(mask_l) = N_ion1_r0_HWF(mask_l) .* W_HWF(mask_l) .* abs(E_d_HWF(mask_l)).^5 .* exp(1i * Phi_LH_l(mask_l));
E_LH_s(mask_s) = N_ion1_r0_HWF(mask_s) .* W_HWF(mask_s) .* abs(E_d_HWF(mask_s)).^5 .* exp(1i * Phi_LH_s(mask_s));

% Perfect phase-matching source is always defined
E_LH_perfect = N_ion1_r0_HWF .* W_HWF .* abs(E_d_HWF).^5;

% Accumulated harmonic fields (now robust)
E_HHG_l       = cumsum(E_LH_l);
E_HHG_s       = cumsum(E_LH_s);
E_HHG_perfect = cumsum(E_LH_perfect);

% Optional: phase-for-plotting deltas, avoiding NaN baselines
dPhi_LH_l = NaN(1, N_z);
dPhi_LH_s = NaN(1, N_z);
if ~isempty(i0l), dPhi_LH_l(mask_l) = Phi_LH_l(mask_l) - Phi_LH_l(i0l); end
if ~isempty(i0s), dPhi_LH_s(mask_s) = Phi_LH_s(mask_s) - Phi_LH_s(i0s); end
