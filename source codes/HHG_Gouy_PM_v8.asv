...
% HHG source depends on I_p, it will switch auto

close all
clear

ParameterFilename = 'Para_Ne_keV_251014.txt'; 
FigureSwitch = 1; 
...
%%   
   ...
   subplot(5,3,3), plot(z/mm,N_parent_HWF*cm^3);
      ...
      % ---- Dynamic title based on ionization stage ik ----
        parent_labels = {'neutral atom (0⁺)', '1⁺ ion', '2⁺ ion', '3⁺ ion', '4⁺ ion'};
        
        if exist('ik','var') && ik >= 1 && ik <= numel(parent_labels)
            parent_label = parent_labels{ik};
        else
            parent_label = 'unknown stage';
        end
        
        title([parent_label, ' density met by the HHG wavefront']);


   subplot(5,3,6), plot(z/mm,Wk_HWF*fs);
      xlabel('z (mm)');
      ylabel('W\_HWF (1/fs)');
      title('ionization rate met by the HHG wavefront');
   subplot(5,3,9), plot(z/mm,Source_HWF);
      xlabel('z (mm)');
